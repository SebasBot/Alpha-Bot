...
module.exports = {
   ...
   async execute(Discord, cliente, interaccion){
      ...
      Collector.on('collect', async i =>{
         ...
         switch(i.values[0]){
            case 'first': 
            await i.editReply({embeds:[Seccion.Seccion1], components: []});
            Collector.stop();
            break
         case 'second': 
            await i.editReply({embeds:[Seccion.Seccion2], components: []});
            Collector.stop();
            break
         case 'third': 
            await i.editReply({embeds:[Seccion.Seccion3], components: []});
            Collector.stop();
            break
         case 'fourth': 
            await i.editReply({embeds:[Seccion.Seccion4], components: []});
            Collector.stop();
            break
         }}catch(error){
         console.log(error); 
